...
def normalize(y_cashbacks, budget):
    """Normalise the cashback data and use some crazy dark magic that i no longer remember or undertand."""
    print(y_cashbacks.shape[0])
    if y_cashbacks.shape[0] < 5:
        return pd.DataFrame(
            [[0] * len(y_cashbacks.columns)] * len(y_cashbacks),
            columns=y_cashbacks.columns,
        )
...
def algo(cash, branch, y_cashbacks, budget):
    """Load the corresponding category models and run it with the given data if the branch is set."""
...
def algo_branch(cash, branches, y_cashbacks, budget):
    """Load the models and run them with the given data if the branch is not set."""
    loss_list = []
    x_new = pd.DataFrame([[i / 15] for i in range(1, 16)])
    y_norm = normalize(y_cashbacks, budget)
...
def run(
    recent: List[float], cashback: float, branch: Literal[*allowed_categories], budget
):
    """Wrapper for running the models"""
    if branch:
        return algo(cashback, branch, pd.DataFrame({"0": recent or []}), budget)
    else:
        return algo_branch(
            cashback, allowed_categories, pd.DataFrame({"0": recent or []}), budget
        )
